...
void GraphicsGL::setupGLMatrices() {
	...
	// glLoadIdentity();
	modelViewMatrix = ProjectionMatrix::Identity();
...
void GraphicsGL::setCamera(GLfloat fovy, GLfloat nearClipping, GLfloat farClipping) {
...
void GraphicsGL::lookAt(GLfloat eyex, GLfloat eyey, GLfloat eyez,
...
void GraphicsGL::updateCamera() {
	...
	projectionMatrix = perspectiveProjection(openGLCamera.fovy,  (double)this->screenWidth / this->screenHeight, openGLCamera.nearClipping, openGLCamera.farClipping);
...
//			  openGLCamera.up[0], openGLCamera.up[1], openGLCamera.up[2]);
	modelViewMatrix *= lookAtMatrix(openGLCamera.eye[0], openGLCamera.eye[1], openGLCamera.eye[2],
			openGLCamera.center[0], openGLCamera.center[1], openGLCamera.center[2],
			openGLCamera.up[0], openGLCamera.up[1], openGLCamera.up[2]);
	modelViewMatrix *= translateMatrix(-openGLCamera.eye[0], -openGLCamera.eye[1], -openGLCamera.eye[2]);
...
void GraphicsGL::draw() {
	...
	auto drawObj = [&](GraphicsObject &obj) {
		...
		ProjectionMatrix modelViewSave = modelViewMatrix;
		// glPushMatrix();

		modelViewMatrix *= translateMatrix(obj.translation[0], obj.translation[1], obj.translation[2]);
		modelViewMatrix *= rotateMatrix(obj.rotationAngle, obj.rotationVector[0], obj.rotationVector[1], obj.rotationVector[2]);
	...
	ProjectionMatrix projectionMatrixSave = projectionMatrix;
	ProjectionMatrix modelViewMatrixSave = modelViewMatrix;
//	glMatrixMode(GL_PROJECTION);
//	glPushMatrix();
//	glLoadIdentity();
	projectionMatrix = orthogonalProjection(0.0 ,this->screenWidth, 0.0, this->screenHeight, 1.0, -1.0);
...
//	glMatrixMode(GL_MODELVIEW);
	...
	projectionMatrix = projectionMatrixSave;
	modelViewMatrix = modelViewMatrixSave;
